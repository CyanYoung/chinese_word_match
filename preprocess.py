...
def save(path, cut_texts, labels):
    head = 'cut_text,label'
        ...
        with open(os.path.join(path_univ_dir, file), 'r') as f:
            for line in f:
                ...
                if text and text not in text_set:
                    ...
                    labels.append(label)
    texts_labels = list(zip(cut_texts, labels))
    shuffle(texts_labels)
    cut_texts, labels = zip(*texts_labels)
    bound = int(len(cut_texts) * 0.9)
    save(path_train, cut_texts[:bound], labels[:bound])
    save(path_test, cut_texts[bound:], labels[bound:])
